...
export function Terminal(props: {
	...
	function handleKeyPress(e: JSX.TargetedKeyboardEvent<HTMLInputElement>) {
		if (e.key === 'Enter') {
			...
			onMouseUp={() => {
				if (inputField.current && scrollBox.current) {
					if (new Date().getTime() - clickTime < 200) {
						...
						if (scrollBox.current.scrollHeight - scrollPos <= 5) {
							scrollBox.current.scrollTop = pos;
						}
					}
				}
			}}
				...
				if (scrollBox.current) {
					setScrollPos(
						(e.target as HTMLDivElement).scrollTop +
							2 +
							parseInt(scrollBox.current.style.height.slice(0, -2), 10)
					);
				}
			}}
			style={{
